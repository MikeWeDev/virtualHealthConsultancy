...
export function getBookingsForDoctor({ doctorName, doctorId }) {
  ...
  const normalizedDoctorId = doctorId != null ? String(doctorId) : null;

  return bookings.filter((booking) => {
    if (normalizedDoctorId) {
      return String(booking.doctorId) === normalizedDoctorId;
    }
    if (normalizedDoctorName) {
      return normalizeName(booking.doctorName) === normalizedDoctorName;
    }
    return false;
  });
}
