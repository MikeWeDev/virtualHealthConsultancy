...
export function getBookingsForDoctor({ doctorName, doctorId }) {
  ...
  if (doctorId != null) {
    return bookings.filter((booking) => String(booking.doctorId) === String(doctorId));
  }

  if (doctorName) {
    const normalizedDoctorName = normalizeName(doctorName);
    return bookings.filter((booking) => normalizeName(booking.doctorName) === normalizedDoctorName);
  }
